...
function localizedValue(
  ...
  return (
    value?.values[locale] ?? value?.values.en ?? value?.values[value.sourceLocale] ?? canonical
  );
}
...
export function ReservationFormManager() {
  ...
  return (
    ...
    >
      <Box sx={{ gridColumn: "1 / -1" }}>
        <Surface>
          <Stack
            ...
            sx={{ alignItems: { sm: "center" }, gap: 2, justifyContent: "space-between" }}
          >
            <Box>
          ...
          >
            ...
            <Stack direction="row">
              ...
              <Button
                ...
                variant="contained"
                onClick={() => setDraft(structuredClone(EMPTY_DRAFT))}
              >
                {t("actions.add")}
              </Button>
            </Stack>
              ...
              {fields.map((field, index) => (
                ...
                >
                  <Box sx={{ minWidth: 0 }}>
                    <Stack direction="row" sx={{ alignItems: "center", flexWrap: "wrap", gap: 1 }}>
                      <Typography sx={{ fontWeight: 700 }}>
                        {localizedValue(field.labelI18n, locale, field.label)}
                      </Typography>
                  ...
                  <Stack direction="row" sx={{ justifyContent: { xs: "flex-end", sm: "initial" } }}>
                    <FieldAction
                      label={`${t("actions.moveUp")}: ${field.label}`}
                      disabled={busy || index === 0}
                      onClick={() => void moveField(index, -1)}
                      icon={<ArrowUp size={18} />}
                    />
                    <FieldAction
                      label={`${t("actions.moveDown")}: ${field.label}`}
                      disabled={busy || index === fields.length - 1}
                      onClick={() => void moveField(index, 1)}
                      icon={<ArrowDown size={18} />}
                    />
                    <FieldAction
                      label={`${t("actions.edit")}: ${field.label}`}
                      disabled={busy}
                      onClick={() => edit(field)}
                      icon={<Pencil size={18} />}
                    />
                    <FieldAction
                      label={`${t("actions.delete")}: ${field.label}`}
                      disabled={busy}
      ...
      <Surface>
        <Stack spacing={2}>
          ...
          {loading ? (
            ...
            preview.map((field) => (
              <PreviewControl key={`${field.source}-${field.key}`} field={field} />
            ))
          )}
        </Stack>
      </Surface>
        ...
        {draft && (
          <DialogContent>
            <Stack spacing={2.25} sx={{ pt: 1 }}>
              <TextField
                autoFocus
                label={t("form.labelEs")}
                value={draft.label.es}
                slotProps={{ htmlInput: { maxLength: 160 } }}
                onChange={(event) =>
                  setDraft({ ...draft, label: { ...draft.label, es: event.target.value } })
                }
              />
              <TextField
              ...
              <TextField
                ...
                value={draft.type}
                onChange={(event) =>
                  setDraft({ ...draft, type: event.target.value as ReservationFormFieldType })
                }
              >
              ...
              {draft.type === "select" && (
                <Stack spacing={1.5}>
                    ...
                    >
                      <Stack direction="row" sx={{ alignItems: "center", gap: 1 }}>
                        <Stack spacing={1.25} sx={{ flex: 1 }}>
                          <TextField
                            label={t("form.optionEs", { number: index + 1 })}
                            value={option.es}
                            onChange={(event) => updateOption(index, "es", event.target.value)}
                          />
                          <TextField
                            label={t("form.optionEn", { number: index + 1 })}
                            value={option.en}
                        ...
                        <Tooltip title={t("actions.removeOption")}>
                          <IconButton
                            ...
                            onClick={() =>
                              setDraft({
                                ...draft,
                                options: draft.options.filter((_, item) => item !== index),
                              })
                            }
                          >
                            <X size={18} />
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setDraft({ ...draft, options: [...draft.options, { es: "", en: "" }] })
                    }
                  >
                    {t("actions.addOption")}
                  </Button>
      ...
      >
        ...
        <DialogContent>
          <Typography>
            {t("delete.body", {
              label: deleteTarget
                ? localizedValue(deleteTarget.labelI18n, locale, deleteTarget.label)
                : "",
            })}
          </Typography>
        </DialogContent>
        <DialogActions>
          ...
          <Button color="error" variant="contained" onClick={() => void removeField()}>
      ...
      <Dialog
        open={publishDialog}
        onClose={busy ? undefined : () => setPublishDialog(false)}
        fullWidth
        maxWidth="xs"
      >
        ...
        <DialogContent>
          <Stack spacing={2}>
            <Typography>
              {publication?.fullyTranslated
                ? t("publication.confirmComplete")
                : t("publication.confirmPending", {
                    count: publication?.missingTranslations.length ?? 0,
                  })}
            </Typography>
            {!publication?.fullyTranslated && (
              <FormControlLabel
                control={
                  <Checkbox
                    checked={approveFallback}
                    onChange={(event) => setApproveFallback(event.target.checked)}
                  />
                }
                label={t("publication.approveFallback")}
              />
            )}
          </Stack>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setPublishDialog(false)}>{t("actions.cancel")}</Button>
          <Button
            variant="contained"
            disabled={busy || (!publication?.fullyTranslated && !approveFallback)}
            onClick={() => void changePublication(true, approveFallback)}
          >
            {t("actions.publish")}
          </Button>
        </DialogActions>
  ...
  function PreviewControl({ field }: { field: ReservationFormPreviewField }) {
    ...
    const options =
      field.optionsI18n?.map((option, index) =>
...
function FieldAction({
  ...
  return (
    <Tooltip title={label.split(":")[0]}>
      <span>
        <IconButton aria-label={label} disabled={disabled} onClick={onClick} color={color}>
          {icon}
        </IconButton>
      </span>
    </Tooltip>
