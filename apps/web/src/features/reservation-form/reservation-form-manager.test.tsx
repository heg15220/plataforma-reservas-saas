...
const second = {
  ...
  label: "Preferencia",
  labelI18n: localized("Preferencia", "Preference"),
  key: "preference",
  position: 1,
};
const preview = [
...
describe("ReservationFormManager", () => {
  it("renderiza el texto localizado y la preview combinada", async () => {
    ...
    expect((await screen.findAllByText("Alergias")).length).toBeGreaterThan(0);
    expect(screen.getByLabelText(/Nombre/)).toBeDisabled();
    expect(screen.getByText("Hay 1 traducción pendiente.")).toBeVisible();
  });

  it("crea un campo con valores ES/EN normalizados", async () => {
    renderWithIntl(<ReservationFormManager />);
    fireEvent.click(await screen.findByRole("button", { name: "Añadir campo" }));
    fireEvent.change(screen.getByLabelText("Etiqueta en español"), {
      target: { value: " Código " },
    });
    fireEvent.change(screen.getByLabelText("Etiqueta en inglés"), { target: { value: " Code " } });
    fireEvent.change(screen.getByLabelText("Clave técnica"), {
      target: { value: "internal_code" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Guardar" }));

    await waitFor(() =>
      expect(createReservationFormField).toHaveBeenCalledWith({
  ...
  it("envía el orden completo al mover un campo", async () => {
    ...
    await waitFor(() =>
      expect(reorderReservationFormFields).toHaveBeenCalledWith([second.id, first.id]),
    );
  });
