            ...
            >
              ...
              <Stack direction={{ xs: "column", sm: "row" }} spacing={1}>
                <Button
                  ...
                  startIcon={<CalendarClock aria-hidden="true" size={17} />}
                  variant="outlined"
                >
                  {t("actions.schedule")}
                </Button>
                <Button
                  ...
                  startIcon={<Pencil aria-hidden="true" size={17} />}
                  variant="outlined"
                >
                  {t("actions.edit")}
                </Button>
            ...
            >
              <Box sx={{ minWidth: 0 }}>
                ...
                <Typography color="text.secondary" variant="body2">
                  ...
                  {t("services.resources", { count: service.employeeResourceIds.length })}
                </Typography>
                <Typography color="text.secondary" variant="body2">
                  {t(`services.bookingModes.${service.bookingMode}`)}
                </Typography>
                {service.allowsAnyAvailableResource && service.employeeResourceIds.length > 0 && (
                  <Typography color="text.secondary" variant="body2">
                    {t("services.anyAvailable")}
                  </Typography>
                )}
              </Box>
              <StatusChip
                label={service.active ? t("services.active") : t("services.inactive")}
...
function ResourceDialog({
  ...
  return (
    <Dialog fullWidth maxWidth="md" onClose={busy ? undefined : onClose} open>
      <DialogTitle>
        {draft.id ? t("resourceForm.editTitle") : t("resourceForm.createTitle")}
      </DialogTitle>
      <DialogContent>
        <Box
          sx={{
        ...
        >
          <TextField
            label={t("resourceForm.type")}
            onChange={(event) => patch({ type: event.target.value as EmployeeResourceType })}
            select
            value={draft.type}
          >
          ...
          <TextField
            ...
            onChange={(event) => patch({ status: event.target.value as EmployeeResourceStatus })}
            select
            value={draft.status}
          >
          ...
          <TextField
            label={t("resourceForm.firstName")}
            onChange={(event) => patch({ firstName: event.target.value })}
            value={draft.firstName ?? ""}
          />
          <TextField
            label={t("resourceForm.lastName")}
            onChange={(event) => patch({ lastName: event.target.value })}
            value={draft.lastName ?? ""}
          />
          <TextField
            label={t("resourceForm.publicAlias")}
            onChange={(event) => patch({ publicAlias: event.target.value })}
            value={draft.publicAlias ?? ""}
          />
          <TextField
            label={t("resourceForm.specialty")}
            onChange={(event) => patch({ specialty: event.target.value })}
            value={draft.specialty ?? ""}
          />
          <TextField
            label={t("resourceForm.photoUrl")}
          ...
          <FormControlLabel
            control={
              <Switch
                ...
                onChange={(event) => patch({ publicVisibility: event.target.checked })}
              />
            }
            label={t("resourceForm.publicVisibility")}
          />
          <TextField
            label={t("resourceForm.description")}
            minRows={3}
            multiline
          ...
          />
          <TextField
            label={t("resourceForm.internalNotes")}
            minRows={3}
            multiline
      ...
      <DialogActions>
        ...
        <Button
          ...
          onClick={onSave}
          startIcon={<Save aria-hidden="true" size={17} />}
          variant="contained"
        >
...
function ScheduleDialog({
  ...
  const patch = (weekday: number, change: Partial<WeeklyHourInput>) =>
    onChange(hours.map((hour) => (hour.weekday === weekday ? { ...hour, ...change } : hour)));
  return (
    <Dialog fullWidth maxWidth="md" onClose={busy ? undefined : onClose} open>
            ...
            {hours.map((hour) => (
              <Box
                ...
                sx={{
                  ...
                  display: "grid",
                  gap: 2,
                  gridTemplateColumns: { md: "120px 1fr 1fr 1fr" },
                  pb: 1.5,
                }}
              >
                ...
                <FormControlLabel
                  control={
                    <Switch
                      ...
                      onChange={(event) => patch(hour.weekday, { available: event.target.checked })}
                    />
                  }
                  label={t("schedule.available")}
                />
                <TextField
                  disabled={!hour.available}
                  label={t("schedule.startsAt")}
                  onChange={(event) => patch(hour.weekday, { startsAt: event.target.value })}
                ...
                <TextField
                  ...
                  onChange={(event) => patch(hour.weekday, { endsAt: event.target.value })}
                  slotProps={{ inputLabel: { shrink: true } }}
                  type="time"
                  value={hour.endsAt ?? ""}
                />
      ...
      <DialogActions>
        ...
        <Button
          disabled={
            busy ||
            loading ||
            hours.some((hour) => hour.available && (!hour.startsAt || !hour.endsAt))
          }
          onClick={onSave}
          startIcon={<Save aria-hidden="true" size={17} />}
          variant="contained"
        >
          {busy ? t("actions.saving") : t("actions.saveSchedule")}
        </Button>
      </DialogActions>
    </Dialog>
  );
...
function ServiceDialog({
  ...
  const toggleResource = (id: string, checked: boolean) =>
    patch({
      resourceIds: checked
        ? [...draft.resourceIds, id]
        : draft.resourceIds.filter((current) => current !== id),
    });
  return (
    <Dialog fullWidth maxWidth="md" onClose={busy ? undefined : onClose} open>
      <DialogTitle>
        {draft.id ? t("serviceForm.editTitle") : t("serviceForm.createTitle")}
      </DialogTitle>
      <DialogContent>
        <Stack spacing={2} sx={{ pt: 1 }}>
          ...
          >
            ...
            />
            <TextField
              label={t("serviceForm.capacity")}
              onChange={(event) => patch({ capacityRequired: Number(event.target.value) })}
              slotProps={{ htmlInput: { min: 1 } }}
              type="number"
              value={draft.capacityRequired}
            />
          </Box>
          <TextField
            label={t("serviceForm.bookingMode")}
            onChange={(event) =>
              patch({ bookingMode: event.target.value as VenueServiceInput["bookingMode"] })
            }
            select
            value={draft.bookingMode}
          >
            <MenuItem value="range">{t("serviceForm.bookingModes.range")}</MenuItem>
            <MenuItem value="exact_time">{t("serviceForm.bookingModes.exact_time")}</MenuItem>
          </TextField>
          <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
            <FormControlLabel
              ...
              label={t("serviceForm.active")}
            />
            <FormControlLabel
              control={
                <Switch
                  checked={draft.allowsAnyAvailableResource}
                  onChange={(event) => patch({ allowsAnyAvailableResource: event.target.checked })}
                />
              }
              label={t("serviceForm.anyAvailable")}
            />
          </Stack>
          <Box>
            <Typography component="h3" variant="h3">
              {t("serviceForm.resources")}
            </Typography>
            {resources.length === 0 ? (
              <Typography color="text.secondary" sx={{ mt: 1 }}>
                {t("serviceForm.resourcesEmpty")}
              </Typography>
            ) : (
              <Box
                sx={{
      ...
      <DialogActions>
        ...
        <Button
          disabled={
            busy || !draft.name.trim() || draft.durationMinutes < 1 || draft.capacityRequired < 1
          }
          onClick={onSave}
          startIcon={<Save aria-hidden="true" size={17} />}
          variant="contained"
...
function Loading({ label }: { label: string }) {
  return (
    <Stack
      aria-label={label}
      role="status"
      sx={{ alignItems: "center", justifyContent: "center", minHeight: 120 }}
    >
...
function emptyService(): ServiceDraft {
  return {
    ...
    capacityRequired: 1,
    active: true,
    allowsAnyAvailableResource: false,
...
function serviceToDraft(service: VenueService): ServiceDraft {
  return {
    ...
    allowsAnyAvailableResource: service.allowsAnyAvailableResource,
    bookingMode: service.bookingMode,
    resourceIds: service.employeeResourceIds,
  };
}
...
function normalizeResource(draft: ResourceDraft): EmployeeResourceInput {
  return {
    ...draft,
    firstName: nullable(draft.firstName),
    lastName: nullable(draft.lastName),
    publicAlias: nullable(draft.publicAlias),
    photoUrl: nullable(draft.photoUrl),
    specialty: nullable(draft.specialty),
    description: nullable(draft.description),
    internalNotes: nullable(draft.internalNotes),
    publicVisibility:
      draft.status === "internal_only" || draft.status === "archived"
        ? false
        : draft.publicVisibility,
  };
}
...
function normalizeService(draft: ServiceDraft): VenueServiceInput {
  return {
    ...
    capacityRequired: draft.capacityRequired,
    active: draft.active,
    allowsAnyAvailableResource: draft.allowsAnyAvailableResource,
