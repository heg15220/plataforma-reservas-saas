...
class ReservationHoldExpirationPolicyTests {
  ...
  private final ReservationHoldExpirationPolicy policy = new ReservationHoldExpirationPolicyImpl();

  @Test
  void expiresExactlyFiveMinutesAfterCreation() {
    Instant createdAt = Instant.parse("2026-07-14T10:00:00Z");

    assertThat(policy.expiresAt(createdAt)).isEqualTo(Instant.parse("2026-07-14T10:05:00Z"));
  }
  ...
  void remainingSecondsNeverBecomesNegative() {
    ...
    assertThat(policy.remainingSeconds(expiresAt, Instant.parse("2026-07-14T10:00:00Z")))
        .isEqualTo(300);
