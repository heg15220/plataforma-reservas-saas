...
class VenuePublicSearchControllerTests {
  ...
  void explicitLocaleTakesPrecedenceAndDelegatesPagination() {
    ...
    when(service.search(SupportedLocale.ES, "cafe", List.of("restaurante"), "Madrid", 2, 12))
        .thenReturn(response);

    assertThat(
            controller
                .search("es", "cafe", List.of("restaurante"), "Madrid", 2, 12, "en-US")
                .getBody())
        .isSameAs(response);

    verify(service).search(SupportedLocale.ES, "cafe", List.of("restaurante"), "Madrid", 2, 12);
  }

  @Test
  void negotiatesSpanishAndFallsBackToEnglish() {
    when(service.search(SupportedLocale.ES, null, null, null, 0, 20)).thenReturn(response("es"));
    when(service.search(SupportedLocale.EN, null, null, null, 0, 20)).thenReturn(response("en"));

    controller.search(null, null, null, null, 0, 20, "es-ES,es;q=0.9");
    controller.search("fr", null, null, null, 0, 20, "es");

    verify(service).search(SupportedLocale.ES, null, null, null, 0, 20);
    verify(service).search(SupportedLocale.EN, null, null, null, 0, 20);
  }
